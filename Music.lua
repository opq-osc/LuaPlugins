...
function ReceiveFriendMsg(CurrentQQ, data)
        ...
        keyWord = data.Content:gsub("点歌", "") --提取关键词 保存到keyWord里
				...
				log.notice("点歌keyWord-->%s",keyWord)
		if keyWord == "" then
							return 1
					end
   ...
   if j.data and j.data.song and j.data.song.list and j.data.song.list[1] then
	 ...
	 pic_url = "http://y.gtimg.cn/music/photo_new/T002R300x300M000"..pic_url..".jpg?max_age=2592000"
		if songID ~= "" then
			response, error_message =
			http.request(
				-- https://api.zsfmyz.top/music/song?songmid=004Z8Ihr0JIu5s&guid=126548448
			"GET",
					 "https://api.zsfmyz.top/music/song",
			{
				query = "guid=126548448&songmid=" ..
					songID,
				headers = {
